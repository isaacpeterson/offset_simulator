...
osim.run <- function(user_global_params = NULL, user_simulation_params = NULL, user_simulated_ecology_params = NULL, user_simulation_dynamics = NULL, loglevel = WARN){
  ...
  input_object <- run_initialise_routines(user_global_params, user_simulation_params, user_simulated_ecology_params, user_simulation_dynamics)
  ...
  for (scenario_ind in params_object$global_params$scenario_subset){
    ...
    current_simulation_params <- input_object$simulation_params_group[[scenario_ind]]
    
    index_object <- initialise_index_object(parcels, 
                                            initial_feature_layers, 
                                            current_simulation_params, 
                                            offset_indexes_to_exclude = c(which(unlist(input_object$offset_probability_list) == 0)), 
                                            dev_indexes_to_exclude = which(unlist(input_object$dev_probability_list) == 0))
    ...
    if (params_object$global_params$number_of_cores > 1 && params_object$global_params$set_seed == TRUE){
      ...
      foreach(realisation_ind = seq_len(params_object$global_params$realisation_num)) %dorng%{
        ...
        run_offset_simulation_routines(input_object, 
                                       current_simulation_params,
                                       index_object,
                                       scenario_ind,
                                       realisation_ind)
