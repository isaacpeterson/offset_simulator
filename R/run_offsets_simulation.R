...
osim.run <- function(user_global_params = NULL, user_simulation_params = NULL, user_feature_params = NULL, loglevel = WARN){
  ...
  for (scenario_ind in global_params$scenario_subset){
    ...
    feature_params_to_use <- select_feature_condition_class_bounds(feature_params, current_simulation_params)
    
    #load objects from files etc
    input_data_object <- build_input_data(global_params, feature_params, current_simulation_params)
    ...
    if (global_params$number_of_cores > 1 && global_params$set_seed == TRUE){
      ...
      foreach(realisation_ind = seq_len(global_params$realisation_num)) %dorng%{
        
        run_offset_simulation_routines(input_data_object, 
                                       current_simulation_params,
                                       global_params, 
                                       feature_params_to_use,
                                       scenario_ind,
                                       realisation_ind)
      }
    } else if ((global_params$number_of_cores > 1) && (global_params$realisation_num > 1)){
      # case when running NON-DETERMINISTIC realisations in parallel
      foreach(realisation_ind = seq_len(global_params$realisation_num)) %dopar%{
      ...
      for (realisation_ind in 1:global_params$realisation_num){
  ...
  if (global_params$save_simulation_outputs == FALSE){
    ...
    unlink(global_params$output_folder, recursive = TRUE)
  }
  
  flog.info('all scenarios done in %s %s', 
            round(difftime(Sys.time(), global_params$strt), 1), 
            units(difftime(Sys.time(), global_params$strt)))
  
  flog.info('all outputs written into %s', global_params$run_folder)
  
  # Clean up the parallel processes if there was more than one
  parallel::stopCluster(global_params$clstr)
}
