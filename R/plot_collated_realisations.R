...
#' @param user_params user configured simulation parameters to use
#' @param loglevel logging level to use, for instance futile.logger::INFO
#' @import futile.logger
#' @export
#' 
#' 
osim.plot <- function(user_plot_params, user_params, loglevel = INFO){
  ...
  run_params_filename <- paste0(simulation_params_folder, '/run_params.rds')
  if (!file.exists(run_params_filename)){
    flog.error(paste('offsetsim run parameter file not found in ', run_params_filename))
    stop()
  } else {
    flog.info('reading %s', run_params_filename)
    
    # get the parameters values for all scenarios in the run
    run_params = readRDS(run_params_filename)
  }
