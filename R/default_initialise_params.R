initialise_default_run_params <- function(){
  default_run_params = list()
  default_run_params$simulated_ecology_user_params_file = vector()
  
  default_run_params$overwrite_default_params = TRUE
  
  # Where simulation outputs will be written
  default_run_params$simulation_folder = paste0(getwd(), '/simulated_data/')
 
  default_run_params$use_simulated_data = TRUE
  
  # Fix the output directory (will overwrite existing files) instead of creating unique 
  default_run_params$unique_simulation_folder = TRUE
  ...
  default_run_params$dev_start = 1
  
  # The time at which development ends
  default_run_params$dev_end = 50
  ...
  default_run_params$total_dev_num = 200
  ...
  default_run_params$save_simulation_outputs = FALSE

  default_run_params$overwrite_existing_landscape_data = TRUE
  # Use inputs previously saved in landscape inputs folder to run current simulation
  default_run_params$run_from_saved = FALSE

  # Saves the landscape data use by simulation into the run specific simulation params folder
  default_run_params$backup_simulation_inputs = TRUE

  # Create an animation of the outputs
  default_run_params$write_movie = FALSE

  # Makes a single pdf at the end of the simulation showing the locatons of all offsets and developments
  default_run_params$write_offset_layer = FALSE
  
  # The total number of layers to use in the offset calcuation (iterating from the start)
  default_run_params$features_to_use_in_offset_calc = 1

  # what subset of features to use in the simulation
  default_run_params$features_to_use_in_simulation = 1 
 
  # The maxoimum number of parcels can be selected to offset a single development
  default_run_params$max_offset_parcel_num = 10

  # Sample the restoration rates from a normal distribution to they vary per parcel and per feature
  default_run_params$sample_restoration_rate = FALSE

  # Sample the decline rates from a normal distribution to they vary per parcel and per feature
  default_run_params$sample_decline_rate = FALSE

  # Stops the offset from delivering any further gains once it has acheived the gains required
  default_run_params$limit_offset_restoration = TRUE

    # The probability per parcel of it being illegally cleared, every parcel gets set to this number - set to zero to turn off
  default_run_params$illegal_clearing_prob = 1e-3

  default_run_params$landscape_evolve_type = 'dynamic'
  
  default_run_params$rescale_ecology_to_feature_subset = TRUE
  
  # logistic decline rate means across simulation features. Sample form a normal distribution with this mean and add noise using  default_run_params$decline_rate_std
  default_run_params$mean_decline_rates = rep(list(-1e-2), length(default_run_params$features_to_use_in_simulation)) 
  
  #set this parameter to zero to yield no noise
  default_run_params$decline_rate_std = rep(list(1e-3), length(default_run_params$features_to_use_in_simulation))

  # Lowest value that the logistic decline curve can reach. It will asypotote to this value
  default_run_params$min_eco_val = 0  
  
  # Max value that the logistic decline curve can reach. It will asypotote to this value
  default_run_params$max_eco_val = 100 
  
  default_run_params$screen_sites_by_size = TRUE 
  
  default_run_params$screen_site_zeros = TRUE
  
  default_run_params$site_screen_size = 20 

  default_run_params$screen_dev_site_zeros = FALSE
  
  default_run_params$screen_offset_site_zeros = TRUE
  # set the random number seed
  default_run_params$set_seed = FALSE
  ...
  # ammount
  default_run_params$match_threshold = 0 
  
  # NOT CURRENTLY USED (Limit the amount of restoration to this percentage of the total available)
  default_run_params$max_restoration_eco_val = 70

  # The mean and the standard deviation of a normal distribution fro which to sample the restoration parameters from
  default_run_params$restoration_rate_params = c(0.02, 0.005)

  default_run_params$apply_offset_exclusion_layer = TRUE
  default_run_params$offset_exclusion_layer_filename = 'protected_areas.rds'
  default_run_params$apply_development_exclusion_layer = TRUE
  default_run_params$dev_exclusion_layer_filename = 'protected_areas.rds'
...
initialise_default_variable_params <- function(){ 
  ...
  default_variable_params$offset_action_params = list()
  ...
  default_variable_params$dev_calc_type = c('future_condition')    #'future_condition', 'current_condition' 
  ...
  default_variable_params$allow_developments_from_credit = TRUE
  ...
  default_variable_params$development_selection_type = 'random'  
  ...
  # developments will always equal the total number (Note sd for this
  # distribution is set in the code the currently isn't user settable)
  default_variable_params$offset_bank_end = 1 

  # THe number parcels to include in banking scheme. These are randomly selected.
  default_variable_params$offset_bank_num = 200 
  ...
  default_variable_params$offset_bank_type = c('credit') #c('parcel_set', 'credit')     
  ...
  default_variable_params$include_potential_developments_in_offset_calc = c(FALSE)
  ...
  # offset (due to future losses that are avoided)
  default_variable_params$include_illegal_clearing_in_offset_calc = c(FALSE)
  ...
  default_variable_params$include_potential_offsets_in_offset_calc = c(FALSE)
  ...
  default_variable_params$dev_counterfactual_adjustment = 'as_offset' 
  ...
  # default_variable_params$include_potential_developments_in_dev_calc = c(FALSE)
  ...
  # default_variable_params$include_illegal_clearing_in_dev_calc = default_variable_params$include_illegal_clearing_in_offset_calc

  # Include future offsets in calculating contribution of avoided gains to the
  # impact of the development. This increases the impact of the development as
  # future gains are avoided
  
  # default_variable_params$include_potential_offsets_in_dev_calc = c(FALSE)
  ...
  default_variable_params$offset_multiplier = 1
