run_offsets_model <- function(global_params, region_params, cfacs, cfac_parcel_trajs, offset_calc_type, current_ecology, decline_rates, parcels, perform_offsets, set_seed){
 ...
 # pdf(filename)
  ...
  xl = t(cbind(paste('mean years to NNL = ', round(mean(NNL_object$NNL_dist))), paste('NNL success = ', NNL_object$success*100, '%' )))
  hist(NNL_object$NNL_dist, main = '', xlab = xl)
  
  hist(outs$collated_parcel_sets_object$offsets$initial_parcel_sums, main = '', xlab = 'selected offset parcel values')
...
#   outs$parcel_trajs <- find_parcel_trajectories(land_parcels, 1:(parcels$land_parcel_num), outs$model_outputs$trajectories[[eco_ind]])
  ...
  if (any(net_cond > 0)){
    NNL_year = min(which(net_cond > 0))
    main_title = paste('Net NNL at', NNL_year, 'years')
...
eco_projected <- function(parcel_vals, min_eco_val, max_eco_val, decline_rate, time_horizon){
  
  t_sh = -1/decline_rate * log( ((parcel_vals - min_eco_val)/(max_eco_val - parcel_vals)))
  eco_shift = min_eco_val + (max_eco_val - min_eco_val)/(1 + exp(-decline_rate*(seq_len(time_horizon) - t_sh)))
  return(eco_shift)
...
predict_parcel_traj <- function(current_ecology, global_params, decline_rate, time_horizon){
  ...
  if (time_horizon == 0){
    ...
    ecology_projected = apply(current_ecology, MARGIN = c(1, 2), FUN = eco_projected, min_eco_val = global_params$min_eco_val, max_eco_val = global_params$max_eco_val, (decline_rate = decline_rate), time_horizon)
...
update_development_object <- function(region_params, global_params, region_ind, ind_available, current_ecology, decline_rates, land_parcels, offset_dims, yr){
  ...
  if (dev_calc_type == 'current_condition'){
    ...
    parcel_vals_used = predict_parcel_vals_multi(predict_type = 'protect', parcel_sums_object$parcel_eco_vals, parcel_indexes, decline_rates, restoration_rate, 
                                                 global_params$min_eco_val, global_params$max_eco_val, eco_dims, time_horizon = (global_params$time_steps - yr))
  }
...
predict_parcel_vals_multi <- function(predict_type, parcel_eco_vals, parcel_indexes, decline_rates, restoration_rate, min_eco_val, max_eco_val, eco_dims, time_horizon){
  ...
  for (parcel_count_ind in seq_len(parcel_num)){
    ...
    for (eco_ind in seq_len(eco_dims)){
      ...
      predicted_parcel = sapply(parcel_eco_vals[[parcel_count_ind]][, , eco_ind], eco_shift, min_eco_val, max_eco_val, decline_rate, time_horizon)
...
write_offset <- function(offset_object, current_ecology, min_eco_val, max_eco_val, ecology_size, restoration_rate, yr){
  ...
  for (parcel_ind in seq_len(parcel_num)){
    ...
    updated_parcel = sapply(current_ecology[current_parcel], eco_shift, min_eco_val = 0, max_eco_val = 100, decline_rate = restoration_rate, time_horizon = 1)
    current_ecology[current_parcel] = updated_parcel 
  }
  return(trajectories)
}
...
find_developments_offsets <- function(region_ind, global_params, region_params, current_ecology, decline_rates, parcels, index_object, perform_offsets, yr){
  ...
  development_object <- update_development_object(region_params, global_params, region_ind, index_object$ind_available[[region_ind]], current_ecology, decline_rates, parcels$land_parcels, global_params$offset_dims, yr) 
...
calc_trajectories_multi <- function(global_params, region_params, current_ecology, decline_rates, parcels, index_object, perform_offsets, record_parcel_sets){
  ...
  for (yr in seq_len(global_params$time_steps)){      #main time loop    
    ...
    current_ecology <- project_current_system(current_ecology, parcels$land_parcels, decline_rates, global_params$min_eco_val, global_params$max_eco_val, time_horizon = 1)
  }
...
project_current_system <- function(current_ecology, land_parcels, decline_rates, min_eco_val, max_eco_val, time_horizon){
  parcel_num = length(land_parcels)
  eco_dims = dim(current_ecology)[3]
  for (eco_ind in seq_len(eco_dims)){
    current_ecology_slice = current_ecology[, , eco_ind] 
    ...
    for (parcel_ind in seq_len(parcel_num)){  
      ...
      } else (updated_parcel = sapply(current_ecology[current_parcel], eco_projected, min_eco_val, max_eco_val, decline_rate, time_horizon))
...
# collated_parcel_sets_object = outs$collated_parcel_sets_object 
# parcel_trajs = outs$parcel_trajs
# assessed_set_index = 5

plot_all_parcel_sets <- function(collated_parcel_sets_object, assessed_set_indexes, global_params){
  ...
  par(tcl = -0.25)
  par(mgp = c(2, 0.6, 0))
  assessed_ind_num = length(assessed_set_indexes)
  for (assessed_set_index in seq_len(assessed_ind_num)){
...
plot_single_net_regional <- function(collated_parcel_sets_object, parcel_trajs, assessed_set_index, global_params, cfac_parcel_trajs){
  ...
  par(tcl = -0.25)
  par(mgp = c(2, 0.6, 0))
  plot_parcel_set_from_collated_object(collated_parcel_sets_object, assessed_set_indexes = assessed_set_index, global_params, global_params$eco_dims)
  ...
  plot(sum_cols(parcel_trajs, 1:dim(parcel_trajs)[2]) - sum_cols(cfac_parcel_trajs, 1:dim(cfac_parcel_trajs)[2]), type = 'l')
...
#   mn = min(lim_vec)
#   
#   overlay_plots(plot_list[[1]], yticks = 'y', axis_lab = TRUE, ylim = c(mn, mx), (heading = "developments"), ylab = '', col_vec = c('black', 'black', 'red'))
#   overlay_plots(plot_list[[2]], yticks = 'n', axis_lab = TRUE, ylim = c(mn, mx), (heading = "offsets"), ylab = '', col_vec = c('black', 'black', 'blue'))
#   overlay_plots(plot_list[[3]], yticks = 'n', axis_lab = TRUE, ylim = c(mn, mx), (heading = "net outcome"), ylab = '', col_vec = c('blue', 'red', 'black'))
# }

# collated_object = outs$collated_parcel_sets_object
...
plot_parcel_set_from_collated_object <- function(collated_object, assessed_set_indexes, global_params, eco_dims){
  ...
  plot_list = vector('list', 3)
  plot_list[[1]] = build_plot_list(collated_developments, assessed_set_indexes, global_params, eco_dims)
  plot_list[[2]] = build_plot_list(collated_offsets, assessed_set_indexes, global_params, eco_dims)
  ...
  mx = max(lim_vec)
  mn = min(lim_vec)
  overlay_plots(plot_list[[1]][, , 1], yticks = 'y', axis_lab = TRUE, ylims = c(mn, mx), (heading = "developments"), ylab = '', col_vec = c('red', 'red', 'red'), lty_vec = c(1, 2, 1), 
                lwd_vec = c(1, 1, 3), legend_vec = c('Restoration Gains', 'Avoided Degredation', 'Net Losses'), legend_loc = 'topleft')
  overlay_plots(plot_list[[2]][, , 1], yticks = 'n', axis_lab = TRUE, ylims = c(mn, mx), (heading = "offsets"), ylab = '', col_vec = c('blue', 'blue', 'blue'), lty_vec = c(1, 2, 1), 
                lwd_vec = c(1, 1, 3), legend_vec = c('Restoration Gains', 'Avoided Degredation', 'Net Gains'), legend_loc = 'topleft')
  overlay_plots(plot_list[[3]][, , 1], yticks = 'n', axis_lab = TRUE, ylims = c(mn, mx), (heading = "net outcome"), ylab = '', col_vec = c('red', 'blue', 'black'), lty_vec = c(1, 1, 1), 
                lwd_vec = c(1, 1, 3), legend_vec = c('Development Losses', 'Offset Gains', 'Net Gains'), legend_loc = 'topleft')
  
}


build_plot_list <- function(collated_object, parcel_set_indexes, global_params, eco_dims){
  plot_list = array(0, c(global_params$time_steps, 3, eco_dims))
  for (eco_ind in 1:eco_dims){
    plot_list[, 1, eco_ind] = sum_cols(collated_object$rest_gains[, , eco_ind], parcel_set_indexes)
    plot_list[, 2, eco_ind] = sum_cols(collated_object$avoided_degs[, , eco_ind], parcel_set_indexes)
    plot_list[, 3, eco_ind] = sum_cols(collated_object$net_gains[, , eco_ind], parcel_set_indexes)
  }
  return(plot_list)
}
  ...
  for (parcel_set_ind in 1:parcel_set_num){
    ...
    for (eco_ind in 1:eco_dims){
      ...
      if (length(current_NNL_yr) > 0){
        NNL_dist[parcel_set_ind, eco_ind] = min(current_NNL_yr) - offset_yr
      } 
    }
  }
  
  failed_parcel_sets = which(NNL_dist == 0)
  
  if (length(failed_parcel_sets) > 0){
    NNL_dist = NNL_dist[-failed_parcel_sets]
  }
  
  NNL_object = list()
  NNL_object$net_gains = net_gains
  NNL_object$NNL_dist = NNL_dist
...
overlay_plots <- function(plot_array, yticks, axis_lab, ylims, heading, ylab, col_vec, lty_vec, lwd_vec, legend_vec, legend_loc){
  
  if (yticks == 'y'){
    plot(plot_array[, 1], axes = axis_lab, type = 'l', main = heading, ylim = ylims, ylab = ylab, col = col_vec[1], lty = lty_vec[1])
  } else if ((yticks == 'n')){
    plot(plot_array[, 1], yaxt = 'n', axes = axis_lab, type = 'l', main = heading, ylim = ylims, ylab = ylab, col = col_vec[1], lty = lty_vec[1])
  }
