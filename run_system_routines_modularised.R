run_offsets_simulation <- function(global_params, program_params, initial_ecology, decline_rates_initial, parcels, banked_offset_vec){ # run the model and return outputs
  ...
  global_object$offset_success_flag = (length(unlist(global_object$dev_object)) > 0)*(length(unlist(global_object$offsets_object)) > 0)          #used to flag whether the model actually found any offset/development matches
...
run_system <- function(global_object, global_params, program_params, current_ecology, decline_rates_initial, parcels){ # main engine for code - returns all development/offset parcel sets, land parcel trajectories etc.
...
#       }
  ...
  #       dev_object <- credited_dev_object
  #     }
  #   }
...
select_inds_to_clear <- function(index_object, land_parcel_num){
  
  parcel_inds <- seq_len(land_parcel_num)                                                              #create vector of all land parcel indicies
  inds_to_remove <- c(unlist(index_object$illegal_clearing), unlist(index_object$developments))             #determine which indicies have already been cleared
...
perform_illegal_clearing <- function(global_object, current_ecology, land_parcel_num, yr, region_ind, current_program_params, 
  ...
  decline_rates <- global_object$decline_rates
  inds_to_clear <- select_inds_to_clear(index_object = global_object$index_object, land_parcel_num = parcels$land_parcel_num)
...
perform_development_from_credit_routine <- function(global_object, current_program_params, current_ecology, dev_credit, region_ind, current_offset_pool, land_parcels,
  ...
  credited_dev_object = global_object$credited_dev_object
  index_object = global_object$index_object
  decline_rates = global_object$decline_rates 
  
  if (current_program_params$use_offset_bank == TRUE){
    net_development_val = sum(unlist(dev_object$parcel_vals_used))
  ...
  if (match_object$development_match_flag == TRUE){
    credited_dev_object = append_current_object(credited_dev_object, 
                                                         current_development_object, 
                                                         append_type = 'as_list')
   ...
   global_object$credited_dev_object = credited_dev_object
   global_object$index_object <- index_object
   global_object$decline_rates <- decline_rates
   return(global_object)
  }
...
prepare_offset_pool <- function(current_offset_pool, current_program_params, index_object, banked_offsets_object, decline_rates, 
...
add_to_bank <- function(global_object, offset_bank_num, current_program_params, yr, current_ecology, region_ind, global_params){
  ...
  global_object$banked_offsets_object = banked_offsets_object
  global_object$index_object = index_object
  global_object$decline_rates = decline_rates
...
initialise_index_object <- function(parcels, global_params){
  ...
  if (global_params$screen_parcel_size == TRUE){
...
initialise_decline_rates <- function(parcels, mean_decline_rates, decline_rate_std, eco_dims){
...
develop_from_credit <- function(current_ecology, dev_credit, global_params, current_program_params, intervention_vec, ind_available, decline_rates_initial, land_parcels, yr, time_horizon){
  ...
  dev_pool_object <- record_current_parcel_set(current_ecology[ind_available],  ind_available,  parcel_num_remaining,  yr,  region_ind)
  ...
  match_object <- select_from_pool(match_type = 'development', match_procedure = 'random', current_pool = unlist(dev_pool_object$parcel_indexes), vals_to_use = unlist(dev_pool_object$parcel_vals_used), dev_credit, 
                                   ...
                                   parcel_for_parcel = TRUE, global_params$eco_dims, max_offset_parcel_num = global_params$max_offset_parcel_num, yr)
...
select_pool_to_match <- function(dims_to_use, ndims, thresh, pool_num, vals_to_use, vals_to_match, match_threshold, current_pool, use_parcel_set_dev_credit, parcel_for_parcel, match_type){
...
select_from_pool <- function(match_type, match_procedure, current_pool, vals_to_use, dev_credit, use_parcel_set_dev_credit, 
  ...
  vals_to_match = offset_multiplier*unlist(vals_to_match_initial[[1]][dims_to_use])
  pool_object <- select_pool_to_match(dims_to_use, ndims, thresh, pool_num, vals_to_use, vals_to_match, match_threshold, 
                                      current_pool, use_parcel_set_dev_credit, parcel_for_parcel, match_type)
  ...
  while(match_flag == FALSE){
    ...
    if (match_procedure == 'euclidean'){
      ...
      match_params = list()
      match_params$match_ind = sample(ncol(vals_to_use), 1)
