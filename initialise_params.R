...
initialise_global_params <- function(){
  ...
  global_params$mean_decline_rates = -5e-3*array(runif(global_params$region_num*global_params$eco_dims), c(global_params$region_num, global_params$eco_dims))
  global_params$time_steps = 50 #number of timesteps simulation will be run
  global_params$max_offset_parcel_num = 5
  global_params$offset_dims_to_use = seq_len(global_params$eco_dims)
  global_params$perform_illegal_clearing = TRUE
  global_params$randomise_dev_nums = FALSE
  global_params$display_object = FALSE
  global_params$offset_thresh = 50
  global_params$parcel_size_lim = 5
  global_params$screen_parcel_size = FALSE
...
initialise_program_params<- function(){
  ...
  program_params$include_potential_developments_in_offset_calc = c(FALSE, TRUE)
  program_params$include_potential_offsets_in_offset_calc = c(FALSE, TRUE)
  program_params$include_illegal_clearing_in_offset_calc = c(FALSE, TRUE)
  
  program_params$include_potential_developments_in_dev_calc = c(FALSE, TRUE)
  program_params$include_potential_offsets_in_dev_calc = c(FALSE, TRUE)
  program_params$include_illegal_clearing_in_dev_calc = c(FALSE, TRUE)
