...
initialise_global_params <- function(){
  ...
  global_params = list()
  global_params$realisation_num = 48 #how many realisations of system to run in parallel
  ...
  global_params$parcel_size_lim = 10 #set min parcel size limits if using global_params$screen_parcel_size
  global_params$screen_parcel_size = TRUE # do not use parcels with fewer cells than defined by global_params$parcel_size_lim
...
initialise_program_params<- function(){
  ...
  program_params$offset_bank_num = 300 # how many parcels to include in banking scheme
  ...
  program_params$restoration_rate = 0.01 
  program_params$offset_time_horizon = c(15, 30)
  program_params$offset_calc_type = c('restoration_gains')  #'current_condition', 'avoided_degs', 'net_gains', 'future_condition', 'restoration_gains', 'restoration_condition_value'
