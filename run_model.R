...
region_params <- initialise_region_params(global_params$region_num, mean_decline_rates = c(0.03, 0.01), offset_calc_type = 'current condition', dev_calc_type = 'current condition')
...
outs$parcel_sets_object <- group_parcel_sets(outs$model_outputs, parcel_sets_counterfactuals, counterfactual_type = 'include clearing and offsets', land_parcels, global_params, decline_rates_initial)
