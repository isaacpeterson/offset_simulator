initialise_global_params <- function(){
  ...
  global_params$offset_time_horizon = 10
  ...
  global_params$dev_vec = find_development_vector(global_params$time_steps, global_params$total_dev_num, sd = 1, min_width = -1, last_dev = global_params$time_steps - 20)   #number of developments that occur per region per development cycle
...
populate_region_list <- function(offset_parcel_selection_type, offset_calc_type, offset_action_type, 
                                 offset_parcel_for_parcel, offset_multiplier, dev_calc_type){
  ...
  current_region_params$offset_calc_type = offset_calc_type #'current' ('predicted') - use current (predicted - i.e. the value the parcel would attain if the parcel was offset) value of offset parcel as offset metric
  current_region_params$offset_action_type = offset_action_type
