initialise_run_params <- function(){
  ...
  run_params$max_offset_parcel_num = 10 #how many parcels can be selected to offset a single development
  ...
  run_params$screen_offset_sites_by_size = FALSE 
  run_params$site_screen_size = 2 # ignore parcels with less than  run_params$parcel_screen_size elements
  
  run_params$screen_dev_sites_by_size = FALSE 
  ...
  run_params$restoration_rate_params = c(0.02)
...
initialise_policy_params <- function(){ #list of variations in policy
  ...
  policy_params$development_selection_type = 'weighted'  #how the development parcels are selected - 'random' or 'weighted'
  ...
  policy_params$offset_action_params = list(c('current_condition', 'maintain'), 
                                            c('net_gains', 'restore')) #, 'restoration_gains', 'avoided_loss', 'current_condition'
